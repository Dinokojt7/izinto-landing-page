...
export default function CommunityMap({ onAddressClick }) {
  ...
  // Sandton coordinates
  const center = { lat: -26.056, lng: 28.060 };
  ...
  useEffect(() => {
    ...
    const initializeMap = () => {
      const googleMap = new google.maps.Map(mapRef.current, {
        ...
        minZoom: 10,
        maxZoom: 16,disableDefaultUI: true,
      ...
      <div className="absolute top-6 left-6 right-6 sm:left-8 sm:right-auto w-auto sm:w-full max-w-lg">
        <div className="bg-white/70 rounded-2xl shadow-xl p-4 sm:p-5 h-auto sm:h-80 flex flex-col justify-center items-center text-center">
          ...
          <button
            ...
            className="bg-blue-700 text-white px-6 sm:px-8 py-3 sm:py-4 w-full rounded-full text-sm sm:text-base font-extrabold italic hover:bg-blue-800 transition-all transform hover:scale-105 active:scale-95 whitespace-nowrap"
          >
            ENTER YOUR ADDRESS
          </button>
        </div>
